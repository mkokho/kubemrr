...
if [[ $(type -t compopt) = "builtin" ]]; then
    complete -o default -F __start_[[kubectl_alias]] [[kubectl_alias]]
else
    complete -o default -o nospace -F __start_[[kubectl_alias]] [[kubectl_alias]]
fi
...
__kubectl_parse_get()
{
    ...
    template="{{ range .items  }}{{ .metadata.name }} {{ end }}"
    local kubectl_out
    if kubectl_out=$(kubemrr -a 127.0.0.1 -p 33033 get --kubectl-flags="$(__kubectl_override_flags)" -o template --template="${template}" "$1" 2>/dev/null); then
        COMPREPLY=( $( compgen -W "${kubectl_out[*]}" -- "$cur" ) )
    fi
}
