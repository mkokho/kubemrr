...
__kubectl_parse_get()
{
    __debug "${FUNCNAME[0]}: getting resource '$1'"
    local template
    template="{{ range .items  }}{{ .metadata.name }} {{ end }}"
    local kubectl_out
    if kubectl_out=$(kmrr get "$1" 2>/dev/null); then
...
if [[ $(type -t compopt) = "builtin" ]]; then
    complete -o default -F __start_kubectl kus
else
    complete -o default -o nospace -F __start_kubectl kus
fi
